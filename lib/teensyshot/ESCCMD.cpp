...
//
//  Set 3D mode state without sending DShot command or saving to ESC flash.
//  Use this when 3D mode is already configured in ESC firmware (e.g. AM32 configurator).
//
int ESCCMD_3D_on_silent( void ) {
  if ( !ESCCMD_init_flag )
    return ESCCMD_ERROR_INIT;
  for ( int i = 0; i < ESCCMD_n; i++ )
    ESCCMD_state[i] |= ESCCMD_STATE_3D;
  return 0;
}
